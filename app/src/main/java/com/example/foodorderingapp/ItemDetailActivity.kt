    ...
    private fun showItemDetails() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                val foodName =
                    cursor.getString(cursor.getColumnIndex(DatabaseHelper.FOOD_NAME_COLUMN))
                val foodPrice =
                    cursor.getString(cursor.getColumnIndex(DatabaseHelper.FOOD_PRICE_COLUMN))
                val foodImage =
                    cursor.getBlob(cursor.getColumnIndex(DatabaseHelper.FOOD_IMAGE_COLUMN))
                val foodDescription =
                    cursor.getString(cursor.getColumnIndex(DatabaseHelper.FOOD_DESCRIPTION_COLUMN))
